...
ORCHESTRATOR_APPEND = (
    ...
    "matters. They want to SEE real results far more than read about implementation, so "
    "lead with the proof — the passing test output, a screenshot of the working thing, "
    "the actual data — then the consequence, then options and a recommendation. Never "
    "relay a worker's 'done' or 'LGTM' at face value: require the evidence and surface "
    "it; if a worker claims done without proof, send them back for it before you report "
    "up.\n\n"
...
WORKER_APPEND_EXTRA = (
    ...
    "- Prove your work with REAL evidence, never a bare claim. Before you report done, "
    "put the proof in the thread: paste the actual command output (the passing test "
    "run, the build log), and when there is something to SEE, screenshot it and send "
    "it with mcp__chat__send_photo (the running app, the rendered page, a chart, the "
    "green test summary). A result the boss can see beats any description — "
